...
public class ModalRouter: NSObject, ChaperoneRouter {
    ...
    fileprivate var animation: Bool = true
    ...
    public func move() {
        if let vc = parent {
            self.presentModal(self.target, from: vc, animation: animation)
        } else {
            self.presentModal(self.target)
        }
    }

    private func presentModal(_ controller: UIViewController, from parent: UIViewController, animation: Bool) {
        parent.present(controller, animated: animation, completion: {})
    }
